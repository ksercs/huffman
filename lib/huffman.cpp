...
void write_encode(My_reader & in, My_writer & out, huffman_table & codes) {
    ...
    while (!in.is_end()) {
        ...
        auto it = codes[byt];
        auto cur_code = it.FF;
        auto cur_code_len = it.SS;
//        smp_logging(byt);
//        std::cout << "BYT : " << byt << " " << cur_code << " " << cur_code_len << "\n";
        ...
        if (cur_code_len < ost) {
            //std::cout << "IF : " << cur_code_len << " " << (int)ost << "\n";
            size_t rub = (64 - cur_code_len);
            cur_code <<= rub >>= rub;
    ...
    //smp_logging(ost);
    out.write_ost(ost % 8); // выведем, сколько лишних битов-нулей записано в файл
}
...
//            std::cout << "--> " << res_byte << " " << res_len << "\n";
            if (it != codes.end()) {
//                std::cout << "HERE " << it -> SS << "\n";
                out.write_byte(it -> SS);
                res_len = 0;
                res_byte = 0;
            }
            res_byte <<= 1;
